...
for ind = 1:n
    if peakLabels(ind) == 1  % only find pause on valid peaks
        thisP = P(ind) * 5;       % set to undecimated size
...
for ind = 1:n
    if troughLabels(ind) == 1  % only find pause on valid peaks
        thisT = T(ind) * 5;       % set to undecimated size
...
for ind = 1:(n - 1)
    if peakLabels(ind) == 1 | peakLabels(ind) == 3 
        plot(P(ind)*5, Q(P(ind)*5), 'xg');
    end;
end;
...
for ind = 1:(n - 1)
    if troughLabels(ind) == 1 | troughLabels(ind) == 3 
        plot(T(ind)*5, Q(T(ind)*5), '+g');
    end;
end;
...
for ind = 1:(n - 1)
    if peakLabels(ind) == 1
        plot(P(ind), Qd(P(ind)), 'xg');
    elseif peakLabels(ind) == 2
        plot(P(ind), Qd(P(ind)), 'xr');
    elseif peakLabels(ind) == 3
        plot(P(ind), Qd(P(ind)), 'xb');
    end;
    text(P(ind), Qd(P(ind)) + th*.5, ['p', int2str(ind)], 'FontSize', 8);
    %plot((P(ind)+200), Qd(P(ind)+200), 'oy');  % end of window
    %plot((P(ind)-200), Qd(P(ind)-200), '.y');  % start of window
end;
n = max(size(T)); % number of troughs
for ind = 1:(n - 1)
    if troughLabels(ind) == 1
        plot(T(ind), Qd(T(ind)), '+g');
    elseif troughLabels(ind) == 2
        plot(T(ind), Qd(T(ind)), '+r');
    elseif troughLabels(ind) == 3
        plot(T(ind), Qd(T(ind)), '+b');
    elseif troughLabels(ind) == 4
        plot(T(ind), Qd(T(ind)), '+y');
    end;
