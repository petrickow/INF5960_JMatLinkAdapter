...
public class frmPreferences extends javax.swing.JInternalFrame {
	...
	/**
	 * The path to the directory in which the wfdb code is installed. This
	 * directory should contain the compiled executables (rdann, etc.).
	 */
	...
	private void initComponents() {// GEN-BEGIN:initComponents
		...
		lblWFDBpathHelp.setText(
				"The WFDB app directory is the directory in which the compiled executables (such as rdann.exe) are located.");
		gridBagConstraints = new java.awt.GridBagConstraints();
	...
	private void cmdChangeFileActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cmdChangeFileActionPerformed
		...
		intResult = frmFileSave.showSaveDialog(null); // this returns if select
														// or cancel was clicked
		if (intResult == JFileChooser.APPROVE_OPTION) {
			txtSave.setText(frmFileSave.getSelectedFile().getAbsolutePath());
		}
	...
	private void cmdAddDBActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cmdAddDBActionPerformed
		// check if the name is already present, and if not add to list box and
		// vectors
		String strName = "";
		int intC = 0;
		...
		if (strName.equals("")) {
			JOptionPane.showMessageDialog(null, "Please enter a database name.", "Error", JOptionPane.ERROR_MESSAGE);
		} else if (jcModel.contains(strName)) {
			JOptionPane.showMessageDialog(null,
					"There already is a database of that name.  Please enter a different name.", "Error",
	...
	private void cmdDeleteActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cmdDeleteActionPerformed
		...
		if (lstClips.getSelectedIndex() == -1) {
			JOptionPane.showMessageDialog(null, "Please select the clip to delete from the list.", "Missing Data Error",
					JOptionPane.ERROR_MESSAGE);
			return;
		}
		strClipName = (String) lstClips.getSelectedValue();

		for (intC = 0; intC < vecNames.size(); intC++) { // take the clip out of
															// the vectors
			if (vecNames.get(intC).equals(strClipName)) {
				vecNames.removeElementAt(intC);
				vecLengths.removeElementAt(intC);
			}
		}
	...
	private void cmdAddActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cmdAddActionPerformed
		// check if name and length are present, if so, add to list box and
		// vectors
		String strName = "";
		String strLength = "";
		int intC = 0;
		...
		strLength = txtLength.getText();
		if (strName.equals("")) {
			JOptionPane.showMessageDialog(null, "Please enter a clip name.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
		if (strLength.equals("")) {
			JOptionPane.showMessageDialog(null, "Please enter a clip length.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
		for (intC = 0; intC < vecNames.size(); intC++) {
			if (vecNames.get(intC).equals(strName)) {
				JOptionPane.showMessageDialog(null,
						"There already is a clip of that name.  Please enter a different name.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
		}
		...
		vecLengths.addElement(strLength); // add new item to vectors
	...
	private void cmdOKActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_cmdOKActionPerformed
		...
		if (strTemp.equals("") == true) {
			JOptionPane.showMessageDialog(null, "The WFDB directory is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else {
			fileTemp = new File(strTemp); // make a file object so can make a
											// few tests on the validity of the
											// path
			if (fileTemp.isDirectory() == false) {
				...
				if (fileTemp.isFile() == false) {
					JOptionPane.showMessageDialog(null, "The WFDB directory does not contain rdann.exe.",
		...
		if (strTemp.equals("") == true) {
			JOptionPane.showMessageDialog(null, "The installation directory is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else {
			fileTemp = new File(strTemp); // make a file object so can make a
			...
			if (fileTemp.isDirectory() == false) {
				...
				if (fileTemp.isFile() == false) {
					JOptionPane.showMessageDialog(null, "The installation directory does not contain puka.jar.",
							"Preference Error", JOptionPane.ERROR_MESSAGE);
					bolValid = false;
				}
				fileTemp = new File(strTemp + "ecgpuwave.exe");
				if (fileTemp.isFile() == false) {
					JOptionPane.showMessageDialog(null, "The installation directory does not contain ecgpuwave.exe.",
							"Preference Error", JOptionPane.ERROR_MESSAGE);
					bolValid = false;
				}
			}
		...
		if (strTemp.equals("") == true) {
			JOptionPane.showMessageDialog(null, "The WFDB data file directory is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else {
			fileTemp = new File(strTemp); // make a file object so can make a
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The ConvertECG directory is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else {
			fileTemp = new File(strTemp); // make a file object so can make a
											...
											// path
			if (fileTemp.isDirectory() == false) {
				JOptionPane.showMessageDialog(null, "The ConvertECG directory is not a valid directory.",
						"Preference Error", JOptionPane.ERROR_MESSAGE);
				bolValid = false;
			} else {
				fileTemp = new File(strTemp + "convertecg.exe");
				if (fileTemp.isFile() == false) {
					JOptionPane.showMessageDialog(null, "The ConvertECG directory does not contain ConvertECG.exe.",
							"Preference Error", JOptionPane.ERROR_MESSAGE);
					bolValid = false;
				}
			}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The sampling frequency is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else {
			...
			if (intTemp <= 0) {
				JOptionPane.showMessageDialog(null,
						"The sampling frequency is not valid.  It must be greater than zero Hz.", "Preference Error",
						JOptionPane.ERROR_MESSAGE);
				bolValid = false;
			}
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The signal units are not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The signal gain is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The ADC resolution is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		} else if ((strTemp.equals("8") == false) & (strTemp.equals("16") == false)) {
			JOptionPane.showMessageDialog(null, "The ADC resolution must be 8 or 16.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The ADC zero level is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The EKG column number is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		strTemp = txtRespiration.getText();
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The respiration column number is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		strTemp = txtTrigger.getText();
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The trigger column number is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "The clip length is not valid.", "Preference Error",
					JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (strTemp.equals("")) {
			JOptionPane.showMessageDialog(null, "Enter the path to where the new preference file should be written.",
					"Preference Error", JOptionPane.ERROR_MESSAGE);
			bolValid = false;
		}
		...
		if (fileChosen.exists()) {
			...
			if (fileChosen.delete() == false) {
				JOptionPane.showMessageDialog(null,
						"The file could not be deleted.  Please delete it or save with a new name.");
				return;
			}
		}
		...
		if (bolValid == true) {
			...
			if (intTemp != 0) {
				JOptionPane.showMessageDialog(null, "The preference.txt file was not written correctly.",
						"Preference Error", JOptionPane.ERROR_MESSAGE);
			}
			try {
				this.setClosed(true);
	...
	private int WritePreferenceFile() {
		// writes out the preference file so the paths will be loaded correctly
		// next time
		int intReturn = 0;
		int intC = 0;

		try {
			FileWriter jcOut = new FileWriter(txtSave.getText()); // writer to
																	// write
																	// preferences
																	// file
			...
			for (intC = 0; intC < vecNames.size(); intC++) {
				jcOut.write("ClipName|" + vecNames.get(intC) + "\n");
			}
			for (intC = 0; intC < vecLengths.size(); intC++) {
				jcOut.write("ClipLength|" + vecLengths.get(intC) + "\n");
			}
			jcOut.write("ColEKG|" + intColEKG + "\n");
			jcOut.write("ColRespiration|" + intColRespiration + "\n");
			jcOut.write("ColTrigger|" + intColTrigger + "\n");
			for (intC = 0; intC < jcDBList.size(); intC++) {
				jcOut.write("dbname|" + jcDBList.get(intC) + "\n");
			}
	...
	public void ReadPreferenceFile() {
		...
		String strInput = "";
		String strToken = "";
		StringTokenizer jcTokenizer;
		Iterator jcIterator;

		filePref = new File("preferences.txt");
		System.out.println("Working Directory = " + System.getProperty("user.dir"));
		if (filePref.isFile() == false) {
			System.out.println("No pref");
			JOptionPane.showMessageDialog(null, "Puka can't find the preference file.", "Error",
					JOptionPane.ERROR_MESSAGE);
			System.exit(1);
		}

		try {
			FileReader jcFReader = new FileReader(filePref); // reader to
																// connect to
																// the file
			BufferedReader jcBReader = new BufferedReader(jcFReader); // another
																		// reader
			while ((strInput = jcBReader.readLine()) != null) {
				jcTokenizer = new StringTokenizer(strInput, "|"); // read
																	// strInput
																	// with | as
																	// the only
																	// delimiter
				strToken = jcTokenizer.nextToken(); // get the first word -
													// parameter name
				if (strToken.equals("WFDBDirectory")) {
					strWFDBPath = jcTokenizer.nextToken();
				} else if (strToken.equals("InstallPath")) {
					strInstallPath = jcTokenizer.nextToken();
				} else if (strToken.equals("WFDBDataPath")) {
					strWFDBDataPath = jcTokenizer.nextToken();
				} else if (strToken.equals("ConvertECGPath")) {
					strConvertECGPath = jcTokenizer.nextToken();
				} else if (strToken.equals("SampleFreq")) {
					strSampleFreq = jcTokenizer.nextToken();
				} else if (strToken.equals("SignalUnits")) {
					strSignalUnits = jcTokenizer.nextToken();
				} else if (strToken.equals("SignalGain")) {
					strSignalGain = jcTokenizer.nextToken();
				} else if (strToken.equals("ADCResolution")) {
					strADCResolution = jcTokenizer.nextToken();
				} else if (strToken.equals("ADCZero")) {
					strADCZero = jcTokenizer.nextToken();
				} else if (strToken.equals("Length")) {
					strLength = jcTokenizer.nextToken();
				} else if (strToken.equals("ClipName")) {
					vecNames.addElement(jcTokenizer.nextToken());
				} else if (strToken.equals("ClipLength")) {
					vecLengths.addElement(jcTokenizer.nextToken());
				} else if (strToken.equals("ColEKG")) {
					intColEKG = new Integer(jcTokenizer.nextToken()).intValue();
				} else if (strToken.equals("ColRespiration")) {
					intColRespiration = new Integer(jcTokenizer.nextToken()).intValue();
				} else if (strToken.equals("ColTrigger")) {
					intColTrigger = new Integer(jcTokenizer.nextToken()).intValue();
				} else if (strToken.equals("dbname")) {
					jcDBList.add(jcTokenizer.nextToken());
				}
			}
			jcFReader.close();
			jcBReader.close(); // get rid of readers
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "The preference file is not valid.  Please check the settings.",
					"Preference Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
		...
		// screen
		// jcIterator = jcDBList.iterator();
		// while (jcIterator.hasNext()) { jcModel.add(0,
		// (String)jcIterator.next()); }
		// lstDBs.setModel(jcModel);
	}
	...
	public static double getClipLength(String strClipName) {
		// returns the length in number of raw signal samples of the clip whose
		// name was passed
		double dblLength = -1;
		int intC = 0;
		String strTemp = "";
		...
		if (dblLength == -1) {
			JOptionPane.showMessageDialog(null, "Couldn't find the clip length.", "Serious Error",
					JOptionPane.ERROR_MESSAGE);
		}
		return dblLength;
	}
	...
	/**
	 * the data column in the raw data file corresponding to the respiration
	 * channel
	 */
	...
	/**
	 * the data column in the raw data file corresponding to the onset trigger
	 * channel
	 */
