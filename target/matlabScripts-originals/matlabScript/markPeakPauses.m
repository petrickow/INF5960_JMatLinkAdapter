...
for i = 1:numPeaks
    thisPeak = validPeaks(i);  % going to find the pause size for this peak
    if validPeaks(1) < validTroughs(1)    % peak-first signal
        if i <= numTroughs
            nextTrough = validTroughs(i);  % peak-first, so next trough has same index
        else
            nextTrough = validTroughs(numTroughs);  % if no next trough assign the last one
        end;
        if i > 1 & i < numTroughs
            prevTrough = validTroughs(i - 1);  % and previous trough has previous index
        else
            prevTrough = validTroughs(1);  % assign to first trough
        end;
    else
        if i < numTroughs
            nextTrough = validTroughs(i + 1);  % trough-first, so next trough has index + 1 number
        else
            nextTrough = validTroughs(numTroughs);
        end;
        if i <= numTroughs
            prevTrough = validTroughs(i);  % and previous trough has same index
        else
            nextTrough = validTroughs(numTroughs);
        end;
    end;
    ...
    while stop == 0 
        ...
        elseif Qd(thisPeak + m) >= Qd(thisPeak)
            ...
            s = ['back to start level - Lp', int2str(i)]    
    ...
    while stop == 0 
        ...
        elseif Qd(thisPeak + m) >= Qd(thisPeak)
            stop = 1;  % height is same as peak point - stop
            s = ['back to start level - Rp', int2str(i)]    
        elseif littles > 30 & ((Qd(thisPeak) - Qd(thisPeak + m)) < mth)
            stop = 1;  % slope not changing much anymore
            m = m + 15; % move back to the start of the 3 littles
            s = ['littles > 30 - Rp', int2str(i)]  
        elseif negatives > 10 & ((Qd(thisPeak) - Qd(thisPeak + m)) < mth)
            stop = 1;  % went downhill for 10 points
            m = m + 10;
            s = ['negatives > 10 - Rp', int2str(i)]  
        else
